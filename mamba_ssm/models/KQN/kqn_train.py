...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Script to train Kqn')
    ...
    parser.add_argument("--n_rnn_hidden", type=int, default=256)
    parser.add_argument("--n_mlp_hidden", type=int, default=256) # Best 256
    ...
    log_file = os.path.join(
        'Test_result/kqn_d{}_{}_test_result.txt'.format(params.n_hidden, params.data_name))
    ...
    for dataset_set_index in range(5):
        ...
        # model
        net = KQN(n_skills=params.n_skill,n_hidden=params.n_hidden,n_mlp_hidden=params.n_mlp_hidden,
                  n_rnn_hidden=params.n_rnn_hidden,n_rnn_layers=params.n_rnn_layers,dropout=params.dropout).to(device)
        net = net.to(device)
        ...
        if os.path.exists(load_model_path):
            ...
            with torch.no_grad():
                for idx in range(test_N):
                    ...
                    skill = skill[:, :-1]
                    lanswer = answer[:, :-1]
                    skill, lanswer, rskill, answer = skill.to(device), lanswer.to(device), rskill.to(device), answer.to(
                        device)

                    pred_res = net(skill, lanswer, rskill, )
        ...
        for epoch in range(params.max_iter):
            ...
            with torch.no_grad():
                for idx in range(val_N):
                    ...
                    skill = skill[:,:-1]
                    lanswer = answer[:,:-1]
                    skill, lanswer,rskill,answer = skill.to(device), lanswer.to(device),rskill.to(device),answer.to(device)
