...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Script to train DTransformer')
    ...
    parser.add_argument('--block', type=int, default=1, help='The number of model layers')
    parser.add_argument('--d_model', type=int, default=128, help='The dimension of the model')
    parser.add_argument('--num_heads', type=int, default=8, help='The head num of Multi-head-attention')
    parser.add_argument('--pid',type=int,default=0,help='The number of problems')
    ...
    log_file = os.path.join(
        'sakt_l{}d{}_{}_test_result.txt'.format(params.block, params.d_model, params.data_name))
    ...
    for dataset_set_index in range(5):
        ...
        # model
        net = DTransformer(params.n_skill, params.pid,n_blocks=params.block).to(device)
        net = net.to(device)
        ...
        train_skill_data, train_answer_data,_ = dat.load_data(train_data_path)
        val_skill_data, val_answer_data ,_= dat.load_data(valid_data_path)
        test_skill_data, test_answer_data,_ = dat.load_data(test_data_path)

        # early stopping
        early_stopping = EarlyStopping(patience=5, verbose=True)
        save_model_file = os.path.join(
            './fold{}/{}/layer{}/dim{}'.format(params.dataset_set_index, params.data_name, params.block,
                                               params.d_model))
        ...
        for epoch in range(params.max_iter):
            ...
            for idx in tqdm(range(train_N)):
                ...
                label_answer = torch.where(answer == -1, torch.tensor([2]), answer)
                answer = torch.where(answer == -1, torch.tensor([0]), answer)
                skill, answer,label_answer = skill.to(device), answer.to(device),label_answer.to(device)

                pred_res, _ = net.get_cl_loss(skill.long(),answer.long())

                loss, y_pred, y_true = kt_loss(pred_res, label_answer)
        ...
        with torch.no_grad():
            ...
            print('fold{}'.format(params.dataset_set_index), 'test auc: ', auc_test,'test acc: ', acc_test, file=log)
