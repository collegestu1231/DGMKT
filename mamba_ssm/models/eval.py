...
class lossFunc(nn.Module):
    def __init__(self, hidden,num_c, kd_loss,max_step, device):
    ...
    def forward(self, logit_c, logit_t, logit_ensemble, batch):
        ...
        # lamda * loss_kd
        loss_kd = self.kd_loss * (torch.sum(torch.abs(p0_enm - p0_c)) + torch.sum(torch.abs(p0_enm - p0_t)))
        ...
        for student in range(batch.shape[0]):
            ...
            # delta = torch.Size([50, 3162])
            # print('你好',torch.sum(delta,dim=-1))这个输出并不是全1，因为有的学生序列不够长，补全了
            a = (((batch[student][:, 0:self.q] -
                   batch[student][:, self.q:]).sum(1) + 1) //
                 2)[1:]  # [49],a 是一个向量,表示学生在每一步的答题情况(正确为 1,错误为 0)
