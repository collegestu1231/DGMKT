...
class MambaKTHeadModel(nn.Module, GenerationMixin):
    ...
    def forward(self,  skill, answer, difficulty=None, inference_params=None, num_last_tokens=0,
                **mixer_kwargs):
        """
        ...
        gate = torch.sigmoid(self.gate_fc(torch.cat([all_stu_h, x], dim=-1)))

        kt_input = gate * all_stu_h + (1 - gate) * x
