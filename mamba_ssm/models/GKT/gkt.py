...
class GKT(nn.Module):
    ...
    def __init__(self, num_c, hidden_dim, emb_size, graph_type="dense", graph=None, dropout=0.5, emb_type="qid", emb_path="",bias=True):
        ...
        if emb_type.startswith("qid"):
            ...
            self.interaction_emb = nn.Embedding(self.res_len * self.num_c, emb_size)
            # last embedding is used for padding, so dim + 1
            self.emb_c = nn.Embedding(self.num_c + 1, emb_size, padding_idx=-1)
        ...
        # Erase & Add Gate
        self.erase_add_gate = EraseAddGate(hidden_dim, self.num_c)
        # Gate Recurrent Unit
    ...
    def _aggregate(self, xt, qt, ht, batch_size):
        ...
        # print(self.interaction_emb)
        # print(torch.max(x_idx_mat))
    ...
    def _predict(self, h_next, qt):
        ...
        y = torch.sigmoid(y)  # [batch_size, num_c]
