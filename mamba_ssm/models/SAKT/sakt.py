...
class SAKT(nn.Module):
    ...
    # 生成模型输入
    def get_inputs(self, skill_inputs, label_inputs):  # b,l
    ...
    # 生成查询向量
    def get_query(self, skill_ids):  # 问题编码层,也就是E \in \R^{(E×d)}
    ...
    def forward(self, skill_ids,skill_inputs, label_inputs,):
        ...
        inputs = self.get_inputs(skill_inputs, label_inputs) # [b,l,2d]
        
        inputs = F.relu(self.lin_in(inputs)) # [b,l,d]
        query = self.get_query(skill_ids) # [b,l,d]
        ...
        outputs = self.dropout(self.attn_layers[0](query, inputs, inputs, self.encode_pos,
                                                   self.pos_key_embeds, self.pos_value_embeds, mask)) #b,l,d
