...
class SAKT(nn.Module):
    ...
    # 生成模型输入
    def get_inputs(self, skill_inputs, stu_h,all_stu_h,label_inputs):  # b,l

        skill_inputs = self.skill_embeds(skill_inputs)
        skill_inputs = skill_inputs + stu_h + all_stu_h
    ...
    # 生成查询向量
    def get_query(self, skill_ids,stu_h,all_stu_h):  # 问题编码层,也就是E \in \R^{(E×d)}

        skill_ids = self.skill_embeds(skill_ids)
        return skill_ids+stu_h+all_stu_h
    ...
    def forward(self,student,skill_ids, skill_inputs, label_inputs,):
        ...
        # sakt
        inputs = self.get_inputs(skill_inputs, stu_h,all_stu_h,label_inputs)  # [b,l,2d]
        inputs = F.relu(self.lin_in(inputs))  # [b,l,d]
        query = self.get_query(skill_ids,stu_h,all_stu_h)  # [b,l,d]
