...
class SAKT(nn.Module):
    ...
    def __init__(self, num_skills, embed_size, num_attn_layers, num_heads,
                 encode_pos, G,max_pos=10, drop_prob=0.05):
        ...
        self.dropout = nn.Dropout(p=drop_prob)
        self.feed_forward = FeedForward(self.embed_size, self.embed_size, drop_prob)
        self.lin_out = nn.Linear(embed_size, 1)
        self.sigmoid = nn.Sigmoid()

        # H Graph
        self.G = G
        emb = nn.Embedding(G.shape[0], self.embed_size)  # 学生数目=4151
        self.stu = emb(torch.LongTensor([i for i in range(G.shape[0])])).cuda()
        self.net = HGNN(in_ch=self.embed_size,
                        n_hid=self.embed_size,
                        n_class=self.embed_size)

    # 生成模型输入
    def get_inputs(self, skill_inputs, stu_h,label_inputs):  # b,l

        skill_inputs = self.skill_embeds(skill_inputs)
        skill_inputs = skill_inputs + stu_h
    ...
    # 生成查询向量
    def get_query(self, skill_ids,stu_h):  # 问题编码层,也就是E \in \R^{(E×d)}

        skill_ids = self.skill_embeds(skill_ids)
        return skill_ids+stu_h

    # 模型的前向传播
    def forward(self,student,skill_ids, skill_inputs, label_inputs,):

        '''
        item_inputs.shape torch.Size([100, 200])
        item_ids.shape torch.Size([100, 200])
        '''
        # print('torch.max(student-1):',torch.max(student-1))
        # print('torch.min(student-1)')
        ...
        inputs = self.get_inputs(skill_inputs, stu_h,label_inputs)  # [b,l,2d]

        inputs = F.relu(self.lin_in(inputs))  # [b,l,d]
        query = self.get_query(skill_ids,stu_h)  # [b,l,d]
