...
from utils import KTLoss, _l2_normalize_adv, extract_floats_from_tensor
from pytorchtools import EarlyStopping
from tqdm import tqdm
from sakt_H import SAKT
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Script to train SAKT with hypergraph')
    ...
    log_file = os.path.join(
        'Test_result/sakt_H_l{}d{}_{}_test_result.txt'.format(params.layer, params.d_model, params.data_name))
    ...
    for dataset_set_index in range(5):
        ...
        net = SAKT(num_skills=params.n_skill,embed_size=params.d_model,num_attn_layers=params.layer,\
                   num_heads=params.num_heads,encode_pos=False,G=G).to(device)
        net = net.to(device)
        ...
        for epoch in range(params.max_iter):
            ...
            for idx in tqdm(range(train_N)):
                ...
                stu_inputs = [torch.cat((torch.tensor([params.n_stu]), stu))[:-1] for stu in stu]
                skill_inputs, label_inputs,stu_inputs = torch.stack(skill_inputs), torch.stack(label_inputs),\
                torch.stack(stu_inputs)

                skill, answer, skill_inputs, label_inputs,stu_inputs = skill.to(device), answer.to(device), \
                    skill_inputs.to(device), label_inputs.to(device),stu_inputs.to(device)



                pred_res, _ = net(stu_inputs,skill, skill_inputs, label_inputs)
