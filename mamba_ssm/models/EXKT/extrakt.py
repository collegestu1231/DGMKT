...
class extraKT(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks, dropout, d_ff=256,
                 ...
                 emb_path="", pretrain_dim=768, num_buckets=32, max_distance=100):
        super().__init__()
        """
        Input:
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, d_feature, n_heads, dropout, kq_same, num_buckets, max_distance, bias=True,
                 emb_type="qid"):
        super().__init__()
        """
    ...
    def forward(self, q, k, v, mask, zero_pad, pdiff=None):
        ...
        if self.emb_type.endswith("avgpool"):
            ...
            scores = attention(q, k, v, self.d_k, mask, self.dropout, zero_pad, gammas, pdiff, alibi=self.alibi,
...
def attention(q, k, v, d_k, mask, dropout, zero_pad, gamma=None, pdiff=None, alibi=None, emb_type=None,
              rel_pos_bias=None, rotary_pe=None):
    """
