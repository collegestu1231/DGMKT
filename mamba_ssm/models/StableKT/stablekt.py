...
class stableKT(nn.Module):
    def __init__(self, n_question, n_pid,
        ...
        self.model = Architecture(n_question=n_question, n_blocks=n_blocks, n_heads=num_attn_heads, dropout=dropout,
                                  ...
                                  model_type=self.model_type, seq_len=seq_len, r=self.r, gamma=self.gamma,
                                  emb_type=self.emb_type, num_buckets=self.num_buckets, max_distance=self.max_distance)
    ...
    def forward(self, q_data,target, pid_data=None,qtest=False, train=False):
        ...
        # # print(q.shape) # torch.Size([64, 0])
        #
        # # print(c.shape) # torch.Size([64, 199])
        # # print(r.shape) # torch.Size([64, 199])
        ...
        y2, y3 = 0, 0
        if emb_type in ["qid", "qidaktrasch", "qid_scalar", "qid_norasch", "qid_sin", "qid_rotary", "qid_t5",
                        "qid_woha", "qid_wha"]:
        ...
        if model_type in {'stablekt'}:
            self.blocks_2 = nn.ModuleList([
                TransformerLayer(d_model=d_model, d_feature=d_model // n_heads,
                                 d_ff=d_ff, dropout=dropout, n_heads=n_heads, kq_same=kq_same, seq_len=seq_len, r=r,
                                 gamma=gamma, emb_type=emb_type, num_buckets=self.num_buckets,
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, d_feature,
        ...
        self.masked_attn_head = MultiHeadAttention(
            d_model, d_feature, n_heads, dropout, kq_same=kq_same, seq_len=seq_len, r=r, gamma=gamma, emb_type=emb_type,
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, d_feature, n_heads, dropout, kq_same, seq_len, r, gamma, emb_type, num_buckets,
                 max_distance, bias=True):
        super().__init__()
        """
        ...
        if emb_type.find("t5") != -1:
            self.rel_pos_bias = T5RelativePositionBias(scale=d_model ** 0.5, causal=True, num_buckets=num_buckets,
    ...
    def forward(self, q, k, v, mask, zero_pad):
        ...
        if self.emb_type.find("woha") != -1:
            scores = attention(q, k, v, self.d_k, mask, self.dropout, zero_pad, alibi=self.alibi,
                               emb_type=self.emb_type, rel_pos_bias=self.rel_pos_bias, rotary_pe=self.rotary_pe)
        else:
            scores = attention(q[:, 0:half_h, :, :], k[:, 0:half_h, :, :], v[:, 0:half_h, :, :], self.d_k,
                               mask, self.dropout, zero_pad, alibi=self.alibi[:, 0:half_h, :, :],
                               emb_type=self.emb_type, rel_pos_bias=self.rel_pos_bias, rotary_pe=self.rotary_pe)
            scores_hakt = attention_hakt(q[:, half_h:, :, :], k[:, half_h:, :, :], v[:, half_h:, :, :], self.d_k,
                                         mask, self.dropout, zero_pad, alibi=self.alibi[:, half_h:, :, :], r=self.r,
                                         gamma=self.gamma, emb_type=self.emb_type, rel_pos_bias=self.rel_pos_bias,
