...
if __name__ == '__main__':
    ...
    parser.add_argument('--size_m', type=int, default=50, help='The head num of Multi-head-attention')
    ...
    log_file = os.path.join(
        'Test_result/dkvmn_HD_d{}_{}_test_result.txt'.format( params.d_model, params.data_name))
    ...
    for dataset_set_index in range(5):
        ...
        # loss Function
        kt_loss = DUALKTLoss(kd_loss=1/(params.batch_size*(params.seqlen-1)))

        # dataset
        if "pid" not in params.data_name:
            dat = DATA(n_question=params.n_skill,
                       seqlen=params.seqlen, separate_char=',', maxstep=params.seqlen)
        else:
            dat = PID_DATA(n_question=params.n_skill,
                           seqlen=params.seqlen, separate_char=',', maxstep=params.seqlen, n_stu=params.n_stu)
