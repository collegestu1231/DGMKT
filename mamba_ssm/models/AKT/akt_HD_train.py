...
from utils import DUALKTLoss, _l2_normalize_adv
from pytorchtools import EarlyStopping
from tqdm import tqdm
from akt_HD import AKT
...
if __name__ == '__main__':
    # train param
    parser = argparse.ArgumentParser(description='Script to train Akt')
    ...
    parser.add_argument('--kq_same', type=bool, default=True, help='')
    ...
    log_file = os.path.join(
        'dual_akt_l{}d{}_{}_test_result.txt'.format(params.layer, params.d_model, params.data_name))
    ...
    for dataset_set_index in range(5):
        ...
        G = adj.cuda()
        net  = AKT(n_question=params.n_skill,  n_blocks=params.layer, d_model=params.d_model,
                    dropout=params.dropout, kq_same=params.kq_same, model_type='akt',G=G).to(device)
        net = net.to(device)
        ...
        for epoch in range(params.max_iter):
            ...
            for idx in tqdm(range(train_N)):
                ...
                skill_answer = torch.where(answer==1,skill+params.n_skill,skill)
                stu = torch.where(stu == -1, torch.tensor([params.n_stu]), stu)
                skill, answer,skill_answer,stu = skill.to(device), answer.to(device),skill_answer.to(device),stu.to(device)
                # print(skill,skill_answer)

                h_logit, d_logit, emseble_logit = net(stu,skill, skill_answer)
            ...
            with torch.no_grad():
                for idx in range(val_N):
                    ...
                    skill_answer = torch.where(answer == 1, skill + params.n_skill, skill)
                    stu = torch.where(stu == -1, torch.tensor([params.n_stu]), stu)
                    skill, answer, skill_answer, stu = skill.to(device), answer.to(device), skill_answer.to(
                        device), stu.to(device)
                    # print(skill,skill_answer)
                    h_logit, d_logit, emseble_logit = net(stu, skill, skill_answer)
                    loss, y_pred, y_true = kt_loss(h_logit, d_logit, emseble_logit, answer)
