...
if __name__ == '__main__':
    ...
    parser.add_argument('--kq_same', type=int, default=1, help='')
    ...
    log_file = os.path.join(
        'akt_D{}d{}_{}_test_result.txt'.format(params.layer, params.d_model, params.data_name))
    ...
    for dataset_set_index in range(5):
        ...
        net  = AKT(n_question=params.n_skill,  n_blocks=params.layer, d_model=params.d_model,
                    dropout=params.dropout, kq_same=params.kq_same, model_type='akt').to(device)
        net = net.to(device)
        ...
        for epoch in range(params.max_iter):
            ...
            for idx in tqdm(range(train_N)):
                ...
                skill, answer,skill_answer = skill.to(device), answer.to(device),skill_answer.to(device)
                # print(skill,skill_answer)
                pred_res, features = net(skill, skill_answer)
