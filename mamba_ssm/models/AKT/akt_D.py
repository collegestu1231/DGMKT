...
class AKT(nn.Module):
    ...
    def forward(self,  q_data, qa_data, ):
        ...
        # D Graph
        mask = torch.ne(q_data, self.n_question).unsqueeze(-1).float()  # [b, l, 1]
        ...
        logits_d = self.sigmoid(logits_d)
        return logits_d[:, 1:],None  # 返回总损失和预测
