...
@triton.jit
def _layer_norm_fwd_1pass_kernel(  # 加速前向传播
...
):  # 执行LayerNorm和RMSNorm的前向传播
    """
    x: 输入张量。
    weight, bias: LayerNorm 的权重和偏置参数。
    eps: 一个小常数，用于防止除以零。
    residual, x1, weight1, bias1: 额外的输入张量及其相关参数。
    dropout_p: dropout 的概率。
    rowscale: 可选的行缩放参数。
    out_dtype, residual_dtype: 输出和残差的目标数据类型。
    is_rms_norm: 是否使用 RMSNorm。
    return_dropout_mask: 是否返回 dropout 掩码。
    """
    if residual is not None:
...
class LayerNormFn(torch.autograd.Function):
    ...
    def forward(
        ...
        # 定义了LayerNorm的Pytorch模块,使用autograd功能实现前向和反向传播
        x_shape_og = x.shape
...
class RMSNorm(torch.nn.Module):
    #  定义了RMSNorm的Pytorch模块,并使用autograd功能实现了前向和反向传播
...
class LayerNormLinearFn(torch.autograd.Function):
    # 定义包含线性层的LayerNorm和RMSNorm的Pytorch模块,并且使用autograd实现了前向和反向传播
    @staticmethod
    @custom_fwd
    def forward(
        ctx,
